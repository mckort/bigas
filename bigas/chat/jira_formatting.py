...
- Before opening a VFA pull request, call lookup_board_releases with project_key=VFA and use pr_base. Never use the current checkout or a released cut (releases[].released or forbidden_pr_bases). If the ticket fix_version is released, ignore it and use pr_base unless the PR is a labeled hotfix. If lookup_board_releases is unavailable, fail closed with `gh release list` / `gh release view vX.Y.Z` instead of guessing from git staging-* branches.
