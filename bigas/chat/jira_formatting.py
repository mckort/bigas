...
- Never open or leave a VFA feature PR on `main` unless the user explicitly asked for a labeled hotfix or a prepare-deploy release merge. If lookup_board_releases returns `pr_base: main` for VFA, that is a misconfig — use `staging-<default_version>` (create it from `main` if missing). Cursor autoCreatePR often targets `main`; retarget with `gh pr edit --base staging-x.y.z` immediately.
