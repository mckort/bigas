    ...
    def create_enhanced_recommendations_prompt(self, ga4_data: Dict, company_context: Dict, target_keywords: Optional[List[str]] = None) -> Optional[str]:
        ...
        if isinstance(ga4_data, dict):
            ...
            for idx, question_data in enumerate(questions):
                ...
                # Handle trend analysis data structure
                if raw_data and 'last_30_days' in raw_data:
                    trend_data = raw_data['last_30_days']
                    if 'summary' in trend_data:
                        summary = trend_data['summary']
                        sessions = max(sessions, summary.get('current_period_total', 0))
                        print(f"🔍 Enhanced AI: Extracted {sessions} sessions from trend analysis data")
                
                # Handle standard GA4 data structure
                elif raw_data and 'rows' in raw_data:
                    rows = raw_data['rows']
    ...
    def generate_enhanced_recommendations(self, ga4_data: Dict, company_context: Dict, target_keywords: Optional[List[str]] = None) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                # No fallback recommendations - return empty list
                recommendations = []
                print(f"🔍 Enhanced AI: No JSON recommendations found, returning empty list")
            ...
            # Return empty recommendations - no fallbacks
            return []
