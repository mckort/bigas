...
class MetaAdsService:
    ...
    def get_campaign_insights(
        ...
        """
        Fetch campaign-level insights for the account and date range.
        account_id: numeric, no act_ prefix (we add it).
        Returns list of insight objects (one per campaign per day).
        """
        aid = (account_id or "").replace("act_", "").strip()
        if not aid:
    ...
    @staticmethod
    def _flatten_insight_row(row: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize one Meta insight row to our schema (date, campaign_id, campaign_name, metrics, derived)."""
    ...
    def normalize_campaign_daily_rows(raw_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for r in raw_rows or []:
            if not isinstance(r, dict):
                continue
            flat = MetaAdsService._flatten_insight_row(r)
            if not flat:
                continue
