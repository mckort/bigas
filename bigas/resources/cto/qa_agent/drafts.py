"""Ephemeral QA proposal drafts (GCS in production, in-memory for tests)."""
...
DRAFT_PREFIX = "qa_drafts/"
DEFAULT_TTL_HOURS = 72


class QADraftStore(Protocol):
...
class InMemoryQADraftStore:
    ...
    def load(self, proposal_id: str) -> Optional[Dict[str, Any]]:
        return self._data.get((proposal_id or "").strip())
...
class GcsQADraftStore:
    ...
    def __init__(
        ...
        *,
        ...
        if self._storage is None and self._bucket is None:
            ...
            if not name:
                raise RuntimeError("STORAGE_BUCKET_NAME is required to store QA proposals")
            from google.cloud import storage as gcs

            self._bucket = gcs.Client().bucket(name)

    def save(self, proposal_id: str, payload: Dict[str, Any]) -> str:
        blob_name = draft_blob_name(proposal_id)
    ...
    def load(self, proposal_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("Failed to load QA proposal %s", blob_name, exc_info=True)
            return None
