...
def openai_tools_to_anthropic(tools: Optional[List[dict]]) -> List[dict]:
    out: List[dict] = []
...
def anthropic_messages(messages: List[dict]) -> tuple:
...
def complete_anthropic_chat(
    ...
    *,
    ...
    if not key:
        raise RuntimeError("ANTHROPIC_API_KEY is not set; cannot call Claude models")
    ...
    if resp.status_code >= 400:
        ...
        raise RuntimeError(f"Anthropic complete failed HTTP {resp.status_code}: {detail}")
    data = resp.json()
    parts = []
    calls = []
