...
- **Chat deploy pipeline:** `bigas/resources/devops/pipeline.py` posts progress messages into the thread and returns `messages` in the chat API response so the UI can render intermediate steps without a full reload. After post-check, pipeline progress messages are marked `metadata.status=complete`. Deploy confirmation state (`pending_deploy` on the thread) is cleared when the user sends an unrelated message. Post-deploy polling is **client-driven**: the thread stores `pending_deploy_poll` after trigger, and the chat UI calls `POST /api/chat/threads/{thread_id}/deploy-poll` every ~20s until GitHub Actions runs finish and health checks are posted (survives Cloud Run scale-to-zero).
