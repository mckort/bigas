...
- **Chat deploy pipeline:** `bigas/resources/devops/pipeline.py` posts progress messages into the thread and returns `messages` in the chat API response so the UI can render intermediate steps without a full reload. After post-check, pipeline progress messages are marked `metadata.status=complete`. Deploy confirmation state (`pending_deploy` on the thread) is cleared when the user sends an unrelated message. **Known limitation:** post-deploy polling runs in a daemon background thread on the Cloud Run instance; if the worker restarts or scales to zero before polling finishes, the automatic post-check message may not appear — ask DevOps for run status or health check again.
