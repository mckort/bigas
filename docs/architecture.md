...
- **`CreateReleaseNotesService`**: Fetches issues by Fix Version (Jira Cloud or internal board via `TicketJiraAdapter` when `USE_INTERNAL_BOARD=true`) and generates customer-facing release notes + comms pack (blog, social drafts). On the internal board, only tickets in **Done** or **Final approval (manual)** on that fix version are included. Optional `create_github_release` creates a semver GitHub Release (`vX.Y.Z`) on the mapped product repo; `mark_released` marks the Fix Version released in Jira or on the board release store.
