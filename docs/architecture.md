...
- **Monday OKR pulse:** `POST /mcp/tools/weekly_okr_pulse` builds a mechanical scoreboard from `/objectives` (KR health, expected vs actual pace, stale currents, unlinked Done with sample size, pending `(manual)` gates, and **1–3 reasoned next steps per red KR** via `bigas/okr/next_steps.py`), then runs the In Progress Objective loop (may open To Do for new levers; references gates instead of cloning; Done does not silence a still-red KR; never auto-starts). Optional LLM comment is appended under the counts and cannot replace them. Posts to Chief of Staff chat (`DISCORD_WEBHOOK_URL_CHIEF`, fallback product). Chat sessions for Chief, Product, and Marketing inject the same scoreboard as required reading (`bigas/okr/priming.py`).
