...
- **Chief of Staff** (`bigas/agents/chief_of_staff.py`): uses `get_llm_client(feature="chat")` with native function calling (Gemini and OpenAI). Falls back to a JSON action loop if the provider rejects tools. Up to 10 tool rounds (`MAX_AGENT_TOOL_ROUNDS`). The user sees only the final answer; tool dumps are not posted as the reply. Chief may call any MCP tool except writes/heavy pipelines in `MUST_DELEGATE_TOOLS` (those become specialist handoffs). Specialists receive domain tools via `AGENT_TOOL_PREFIXES`; `create_jira_issue`, `lookup_jira`, and `search_jira` are always included (`SHARED_AGENT_TOOLS`).
