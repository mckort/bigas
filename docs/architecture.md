...
- **`LookupJiraService`**: MCP tool `lookup_jira` — read-only lookup of an issue (description, human comments, parent Epic) and/or open Epics in a project. Agents use this for filing context, then decide whether a new Task belongs under an Epic or should be standalone. Chat routing treats `lookup_jira` and `create_jira_issue` as **shared** (`SHARED_AGENT_TOOLS` in `chief_of_staff.py`): every specialist and Chief of Staff can call them; COS does not hand them off to Product. A successful lookup is humanized with status, description, comments, then the ticket link and Move button — not the link alone.
- **`ReviewJiraIssueService`**: MCP tool `review_jira_issue` — Product Manager critique of Brief + AI Research/Plan + human comments. Posts or updates one marked Jira comment (`[bigas-pm-review]`). Product-owned (`MUST_DELEGATE_TOOLS`); COS delegates rather than running the review itself. Chat uses the `review` field as the user-facing reply (analysis first, ticket link + Move button as footer).
...
- **Chief of Staff** (`bigas/agents/chief_of_staff.py`): uses `get_llm_client(feature="chat")` and delegates to Marketing/Product/CTO/DevOps via MCP tool calls or OpenAI function calling. Specialists receive domain tools via `AGENT_TOOL_PREFIXES`; `create_jira_issue` and `lookup_jira` are always included (`SHARED_AGENT_TOOLS`) and are also on the Chief of Staff direct-tool allowlist. `review_jira_issue` is Product-only and is delegated.
...
- **Discord → chat**: `post_to_discord` / `post_long_to_discord` in `discord_webhook.py` mirror each notification to the matching specialist thread (and the activity feed) via `mirror_discord_message`. Mapping is by `chat_agent_id` or webhook env (`_PRODUCT` → Product Manager, `_CTO`/`_QA` → CTO, `_MARKETING` → Marketing Analyst, `_DEVOPS` → DevOps, `_CHIEF` → Chief of Staff). Long Discord posts become one chat message (not one per chunk). Discord “on its way…” pings are not mirrored to chat. `CHAT_ENABLED=false` skips thread posts.
