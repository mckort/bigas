...
- **`SearchJiraService`**: MCP tool `search_jira` — read-only JQL search scoped to portfolio projects. Use when the user described a filter instead of issue keys. Requires Jira Cloud (not the internal board). Chat routing treats `lookup_jira`, `search_jira`, and `create_jira_issue` as **shared** (`SHARED_AGENT_TOOLS` in `chief_of_staff.py`): every specialist and Chief of Staff can call them; COS does not hand them off to Product.
