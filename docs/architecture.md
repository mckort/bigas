...
- **Auth**: `bigas/chat/auth.py` — Firebase JWT verification or dev token (`CHAT_AUTH_MODE=dev`). Chat access in Firebase mode is limited to `CHAT_ALLOWED_EMAILS` (falls back to `CHAT_ADMIN_EMAILS`). Agent config updates require an email listed in `CHAT_ADMIN_EMAILS`.
