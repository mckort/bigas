...
- **Staging workflow (BIG-42):** `PROJECT_BRANCH_MAPPING` routes implement/auto-merge PRs to `staging` vs `main` per Jira project. `hotfix` label bypasses staging. Active Fix Versions are assigned when implement starts (Jira API or internal board env).
