...
- **`LookupJiraService`**: MCP tool `lookup_jira` — read-only lookup of an issue (including parent Epic) and/or open Epics in a project. Agents use this for context, then decide whether a new Task belongs under an Epic or should be standalone. Chat routing treats both tools as **shared** (`SHARED_AGENT_TOOLS` in `chief_of_staff.py`): every specialist and Chief of Staff can call them; COS does not hand them off to Product.
...
- **Chat deploy pipeline:** `bigas/resources/devops/pipeline.py` emits progress as events on an agent **task** (`bigas/chat/tasks.py`, A2A-aligned states). Chat threads are projections of that task, so COS and DevOps see the same pre-check → deploy → post-check. After post-check, progress messages are marked `metadata.status=complete`. If a workflow fails (not cancelled/skipped), the pipeline fetches failed job logs, posts a diagnosis, and launches a CTO Cursor cloud agent (`fix_failed_deployment` / `launch_failed_deploy_fix`) to open a fix PR — still inside the same DevOps task (no child tasks). High-risk deploys use task state `input-required`. Post-deploy polling is **client-driven** plus a scheduler backup: the UI calls `POST /api/chat/threads/{thread_id}/tasks/tick` (alias: `/deploy-poll`) every ~20s; Cloud Scheduler should hit `POST /api/agents/tick-tasks` every minute so nudges and post-check survive Cloud Run scale-to-zero and a closed browser. A working task gets one canned nudge after 5 minutes. The delegator sets `review_result`; COS runs an LLM on the final artifact only when that flag is true.
...
- **Chief of Staff** (`bigas/agents/chief_of_staff.py`): uses `get_llm_client(feature="chat")` and delegates to Marketing/Product/CTO/DevOps via MCP tool calls or OpenAI function calling. Delegation creates an agent **task** (`bigas/chat/tasks.py`) with `review_result` chosen by the delegator. Status events are projected into COS and specialist threads with no LLM; a final artifact can trigger one COS review pass (`bigas/agents/task_runtime.py`). Specialists receive domain tools via `AGENT_TOOL_PREFIXES`; `create_jira_issue` and `lookup_jira` are always included (`SHARED_AGENT_TOOLS`) and are also on the Chief of Staff direct-tool allowlist.
- **Async callbacks**: sub-agents POST to `/api/chat/callback` with `thread_id` / `task_id` and optional `final: true` to append a status event or complete the task. The UI polls `GET /api/chat/threads/<id>/messages` and `POST .../tasks/tick` while `task_poll_active` is set. Scheduler backup: `POST /api/agents/tick-tasks` (same auth as evaluate-goals).
