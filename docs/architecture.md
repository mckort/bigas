...
- **`CreateJiraIssueService`**: MCP tool `create_jira_issue` — creates Task/Bug issues via shared `JiraClient` (`JIRA_BASE_URL`, `JIRA_EMAIL`, `JIRA_API_TOKEN`). Returns issue key + browse URL. Optional `marketing=true` adds the Jira label `marketing` for marketing-related work. Optional `parent_epic_key` links the issue to a known goal Epic (`JIRA_EPIC_LINK_FIELD`, default `parent`); omit it to create a standalone ticket. Invalid parents are dropped and the issue is created without a parent.
- **`LookupJiraService`**: MCP tool `lookup_jira` — read-only lookup of one or more issues (including parent Epic) and/or open Epics in a project. `issue_key` accepts a browse URL, comma-separated keys, or a range (`BIG-15 to BIG-18`). Multi-issue lookups skip the open-Epic dump unless `project_key` is passed. Agents use this for context, then decide whether a new Task belongs under an Epic or should be standalone.
- **`SearchJiraService`**: MCP tool `search_jira` — read-only JQL search scoped to portfolio projects. Works on the **internal Bigas board** (subset JQL: project, type, status/statusCategory, text, labels, created/updated/resolved dates) and on Jira Cloud. Use when the user described a filter instead of issue keys. In the reasoning-based chat approach, every specialist and Chief of Staff can call `lookup_jira`, `search_jira`, `fetch_github_activity`, and `create_jira_issue` directly (no domain-based tool filtering).
