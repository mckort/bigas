...
- **Deploy trigger:** `POST /mcp/tools/trigger_deployment` dispatches GitHub Actions workflows configured in `BIGAS_DEPLOY_WORKFLOW_MAP`. VFA uses `deploy-backend.yml` + `deploy-web.yml` on the product repo. BIG dispatches `deploy.yml` on `mckort/bigas` (Cloud Run via `./deploy.sh`). GPWW/FYDA/REM/MYL/FRI dispatch `deploy.yml` on `BIGAS_DEPLOY_REPO_MAP` (the VM infra repo) on the infra default branch and pass `site` plus product `ref` so only that app is pulled at the requested branch. FRI defaults to `mckort/frimaninvestments` + `site=frimaninvestments`. Requires `GITHUB_TOKEN` with Actions write access and `workflow_dispatch` on target workflows.
