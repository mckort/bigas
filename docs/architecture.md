...
- **Monday OKR pulse:** `POST /mcp/tools/weekly_okr_pulse` builds a mechanical scoreboard from `/objectives`, then runs the In Progress Objective loop (`okr_plan` / `propose_next_steps`): 1–3 reasoned actions per at-risk KR, optional new To Do cards, never auto-starts. Steps are stored on the Objective as `okr_next_steps` for `/objectives` **This week**. Optional LLM comment only when `include_comment` is true. Posts to Chief of Staff chat (`DISCORD_WEBHOOK_URL_CHIEF`, fallback product). Chat priming injects the mechanical scoreboard only (`bigas/okr/priming.py`).
