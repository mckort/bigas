...
- **`HotfixService` (BIG-42)**: MCP tool `cherry_pick_hotfix` — finds a merged staging PR for a Jira issue, cherry-picks onto `main` (GitHub Actions `cherry_pick.yml` or Git API fallback), and opens a hotfix PR. Works with Jira and the internal board (`PROJECT_BRANCH_MAPPING`, `BIGAS_PROJECT_ACTIVE_FIX_VERSION`).
