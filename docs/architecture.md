...
- **`CreateJiraIssueService`**: MCP tool `create_ticket` (alias `create_jira_issue`) — creates Task/Bug issues on the internal board, or via shared `JiraClient` when Jira is the source. Before create, checks open issues in the project for a normalized/near-duplicate title and returns the existing key when found (`deduplicated: true`) so MCP retries do not duplicate cards or launch a second In Progress (AI) agent. Returns issue key + browse URL. Optional `marketing=true` adds the label `marketing`. Optional `parent_epic_key` links the issue to a known goal Epic; omit it to create a standalone ticket. Optional `status` sets the board column.
...
- **Monday OKR pulse:** `POST /mcp/tools/weekly_okr_pulse` builds a mechanical scoreboard from `/objectives` (KR health, expected vs actual pace, stale currents, unlinked Done with sample size, pending `(manual)` gates), then runs the In Progress Objective loop (may open To Do; skips Done / already-shipped work; never auto-starts). Optional LLM comment is appended under the counts and cannot replace them. Posts to Chief of Staff chat (`DISCORD_WEBHOOK_URL_CHIEF`, fallback product). Chat sessions for Chief, Product, and Marketing inject the same scoreboard as required reading (`bigas/okr/priming.py`).
