...
- **Chief of Staff** (`bigas/agents/chief_of_staff.py`): uses `get_llm_client(feature="chat")` and delegates to Marketing/Product/CTO/DevOps via MCP tool calls or OpenAI function calling. Specialists and Gemini COS run a think → tool → observe → answer loop (`_run_json_agent_loop`, up to 5 tool rounds). The user sees only the final answer; tool dumps are not posted as the reply. GPT COS uses the OpenAI function-calling loop. Specialists receive domain tools via `AGENT_TOOL_PREFIXES`; `create_jira_issue` and `lookup_jira` are always included (`SHARED_AGENT_TOOLS`) and are also on the Chief of Staff direct-tool allowlist.
