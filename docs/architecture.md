...
- **Chief of Staff** (`bigas/agents/chief_of_staff.py`): uses `get_llm_client(feature="chat")` and delegates to Marketing/Product/CTO/DevOps via MCP tool calls or OpenAI function calling. Specialists receive domain tools via `AGENT_TOOL_PREFIXES`; `create_jira_issue` is always included (`SHARED_AGENT_TOOLS`) and is also on the Chief of Staff direct-tool allowlist.
