...
- **`CreateReleaseNotesService`**: Fetches Jira issues by Fix Version and generates customer-facing release notes + comms pack (blog, social drafts). Optional `create_github_release` creates a semver GitHub Release (`vX.Y.Z`) on the mapped product repo; `mark_released` marks the Fix Version released in Jira.
- **`HotfixService` (BIG-42)**: MCP tool `cherry_pick_hotfix` — finds a merged staging PR for a Jira issue and dispatches GitHub Actions `cherry_pick.yml` to cherry-pick onto `main` and open a hotfix PR. Works with Jira and the internal board (`PROJECT_BRANCH_MAPPING`, `BIGAS_PROJECT_ACTIVE_FIX_VERSION`).
- **Staging workflow (BIG-42):** `PROJECT_BRANCH_MAPPING` is a prefix (`VFA:staging` → `staging-0.2.3` when the ticket has that fix version). Implement creates the versioned branch from `main` if missing. Chat-opened PRs against `main` or unversioned `staging` are retargeted to `staging-x.y.z` (created from `main` with the review token if missing). If that branch cannot be created, CTO auto-merge refuses to squash onto `main`. Ticket keys are read from PR title or branch only, not body prose. `prepare deploy` merges that branch only. After that merge lands on `main`, the cut is PR-locked (`pr_locked` + GitHub ruleset) so new work targets the next staging branch. After a successful versioned deploy, newer `staging-*` branches are rebased onto `main` (`rebase_release.yml`; merge API + Cursor on conflict if the workflow is missing). `hotfix` label bypasses staging. Fix versions are assigned when implement starts, when a PR without a key auto-creates a board ticket (versioned feature branch, board default, or lowest unreleased), and again at Final approval if the ticket is still untagged. CTO auto-merge squash uses the PR title plus number so the ticket key lands on `main`.
- **`CreateJiraIssueService`**: MCP tool `create_ticket` (alias `create_jira_issue`) — creates Task/Bug/Feature/Improvement issues on the internal board, or via shared `JiraClient` when Jira is the source. Returns issue key + browse URL. Optional `marketing=true` adds the label `marketing`. Optional `parent_epic_key` links the issue to a known goal Epic; omit it to create a standalone ticket. Optional `status` sets the board column.
- **`LookupJiraService`**: MCP tool `lookup_ticket` (alias `lookup_jira`) — read-only lookup of one or more issues (including parent Epic) and/or open Epics in a project. `issue_key` accepts a browse URL, comma-separated keys, or a range (`BIG-15 to BIG-18`). Multi-issue lookups skip the open-Epic dump unless `project_key` is passed. Agents use this for context, then decide whether a new Task belongs under an Epic or should be standalone.
- **`SearchJiraService`**: MCP tool `search_tickets` (alias `search_jira`) — read-only JQL search scoped to portfolio projects. Works on the **internal Bigas board** (subset JQL: project, type, status/statusCategory, text, labels, created/updated/resolved dates) and on Jira Cloud. Use when the user described a filter instead of issue keys. Every specialist and Chief of Staff can call `lookup_ticket`, `search_tickets`, `fetch_github_activity`, and `create_ticket` directly.
- **`ProactiveGoalEngine` (BIG-12):** `POST /api/agents/evaluate-goals` — Cloud Scheduler webhook that always requires auth (`X-Bigas-Access-Key` / `Authorization: Bearer`, or legacy `CRON_SECRET`). Also invoked immediately when an **Epic** is dragged into an AI column (`jira_status_automation` skips Research/Design/Implement for Epics). Reads Jira Epics in Research / Plan / In Progress including the board names `Research and describe (AI)`, `Design and plan (AI)`, `In Progress (AI)` (`BIGAS_GOAL_EPIC_STATUSES`), gathers Jira + GitHub + GA4 context for In Progress goals, posts a Chief of Staff progress report to Discord/chat, delegates to specialist agents, and creates backlog Tasks via `CreateJiraIssueService` (spam prevention via open-issue list). See README “Proactive Goal Engine”.
- **`ProgressUpdatesService`**: Jira issues moved to Done in a window → team progress “coach” message (e.g. to Discord).
- **Monday OKR pulse:** `POST /mcp/tools/weekly_okr_pulse` builds a mechanical scoreboard from `/objectives` (KR health, expected vs actual pace, stale currents, unlinked Done with sample size, pending `(manual)` gates), then runs the In Progress Objective loop (reasoned next steps per at-risk KR; may open To Do; human gates via `existing_key`; skips Done / already-shipped work; never auto-starts). Opt-in LLM comment (`include_comment: true`) is appended under the counts and cannot replace them. Posts to Chief of Staff chat (`DISCORD_WEBHOOK_URL_CHIEF`, fallback product). Chat sessions for Chief, Product, and Marketing inject the same scoreboard as required reading (`bigas/okr/priming.py`).
...
- **Pre-flight risk check:** `POST /mcp/tools/check_deployment_risk` compares git refs (latest `deploy-backend-*` / `deploy-web-*` / `deploy-app-*` tags → default branch by default; never silent `main → main` when no prod version exists) and flags database migrations, dependency lockfiles, and deploy/infrastructure config changes. DevOps chat runs a pipeline that posts pre-check → deploy → post-check progress in the thread.
- **Deploy trigger:** `POST /mcp/tools/trigger_deployment` dispatches GitHub Actions workflows configured in `BIGAS_DEPLOY_WORKFLOW_MAP`. VFA uses `deploy-backend.yml` + `deploy-web.yml` on the product repo. BIG dispatches `deploy.yml` on `mckort/bigas` (Cloud Run). GPWW/FYDA/REM/MYL/FRI dispatch `deploy.yml` on `BIGAS_DEPLOY_REPO_MAP` (the VM infra repo, default `mckort/gcp-single-vm-webstack` for FRI in code) on the infra default branch and pass `site` plus product `ref` so only that app is pulled at the requested branch. Requires `GITHUB_TOKEN` with Actions write access and `workflow_dispatch` on target workflows.
...
- **Prepare deploy (versioned):** `bigas/resources/devops/prepare.py` handles `prepare deploy PROJECT VERSION` and the composer shortcut (`GET /api/chat/projects`). It compares the versioned feature branch (`staging-0.2.3`) to production, always posts why a release PR is or is not opened, opens or reuses a PR (cut ticket keys in title/body/squash commit), runs CTO review + autofix, and merges onto `main`. Once the cut is on `main`, it locks that version for new PRs (`pr_locked` + `lock_released_feature_branch`) without closing the board release. When the inline review has leftover Minor, prepare launches up to two nits-only autofix rounds; leftover nits after those rounds or the five-round cap are accepted and the release PR can merge. If Actions later refreshes the marked comment to a fully clean review, `/deploy-poll` can still merge from `wait_actions_review`. Then it reports board tickets on that release (Done and Final approval are in the cut), reconciles those keys against commits from prod deploy tags → `main`, posts what `yes` will deploy, and runs `check_deployment_risk`. If the merge was skipped, cut tickets are missing from the prod→`main` git range, and there are no shipping commits (prod equals `main`), it stops instead of asking to deploy. When `main` is ahead of prod it still asks, with the usual missing-from-git warning. Otherwise it always asks before deploying `main` (even when risk is low and the cut matches git), and calls out leftover open tickets, cut tickets missing from git, or extra commits without a ticket on that version. A **yes** after prepare always starts that cut's deploy (leftover `pending_release_notes` from an earlier version are cleared and cannot steal the confirm). Progress posts Actions run links, then each workflow as it finishes, then post-check. Social/blog drafts are offered only after workflows are green and site health is ok, using the cut version from the poll. A successful versioned deploy closes the board release with `create_next_if_missing=False` (carry-forward only leftover open cards — Final approval stays on the cut), posts `create_release_notes`, and can store an X Approve/Decline draft from the generated copy. `pending_prepare_poll` and `pending_release_notes` live on the thread; the same `/deploy-poll` endpoint advances the PR wait.
- **Staging environments:** `bigas/resources/devops/gpw_pipeline.py` handles `prepare staging`, `update staging`, `teardown staging`, and `prepare deploy <PROJECT>` for every entry in `DEFAULT_STAGING_ENVS` (override or extend with `BIGAS_STAGING_ENV_MAP`). This is separate from versioned `prepare deploy PROJECT VERSION`. With one configured project the key may be omitted. GPW-PROD is the built-in entry for `Green-Promo-Wear-Global/GPW`: prepare staging reviews `develop` against `production`, then dispatches `prepare-staging.yml` to build the staging stack from the live production image plus a production database export. Update staging deploys that reviewed SHA, runs migrations, and leaves maintenance on if the post-check fails. Teardown destroys only the staging Terraform stack after a yes. Prepare deploy requires that rehearsal, takes a production backup, asks, then deploys with maintenance on until the post-check is green. A failed update or production deploy launches the CTO autofix agent on the candidate branch and does not turn maintenance off.
...
- **Chief of Staff** (`bigas/agents/chief_of_staff.py`): uses `get_llm_client(feature="chat")` with native function calling (Gemini and OpenAI). Falls back to a JSON action loop if the provider rejects tools. Up to 10 tool rounds (`MAX_AGENT_TOOL_ROUNDS`). The user sees only the final answer; tool dumps are not posted as the reply.
  
  **Reasoning-based approach**: All agents have access to all tools. The model reasons about which tools to use and when to involve specialists based on the task context, rather than hardcoded routing rules. Specialists are involved via `consult_specialist` when their domain expertise would genuinely improve the outcome. System prompts emphasize step-by-step reasoning and principled decision-making over rigid rules.
...
- **Discord → chat**: `post_to_discord` / `post_long_to_discord` in `discord_webhook.py` mirror each notification to the matching specialist thread (and the activity feed) via `mirror_discord_message`. Mapping is by `chat_agent_id` or webhook env (`_PRODUCT` → Product Manager, `_CTO`/`_QA` → CTO, `_MARKETING` → Marketing Analyst, `_DEVOPS` → DevOps, `_CHIEF` → Chief of Staff). Long Discord posts become one chat message (not one per chunk). Discord “on its way…” pings are not mirrored to chat. `mirror_thread=False` keeps the activity-feed card without posting into the specialist thread (used for CTO PR review, autofix, Ready to merge, Final approval, and auto-merge cards). `CHAT_ENABLED=false` skips thread posts.
...
- **Scheduling**: `POST /tasks/eval/vfa-living-analysis` (Cloud Scheduler job `bigas-eval-vfa-models`: `0 16 1 * *`, Europe/Stockholm, body `{"every_n_weeks":1}`) or `python scripts/run_eval.py --pack vfa-living-analysis`.
