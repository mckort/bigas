...
Optional auto-merge: set Bigas env `BIGAS_CTO_AUTO_MERGE=true` to squash-merge when the review is fully clean, or leftover Minor remains after two nits-only autofix rounds or the five-round cap. The Actions loop passes `auto_merge: true`; chat/MCP `review_and_comment_pr` does not merge unless that flag is set. Discord/Activity **PR auto-merged**, or **PR auto-merge enabled** if checks are still pending. Squash commit headline is the current PR title plus number so the ticket key survives GitHub’s “use first commit” default. The card includes the Jira issue label when the PR is linked (`BIG-15` — summary). Draft PRs are marked ready for review first. Default is off. Repo must allow auto-merge. PR review, autofix, Ready to merge, Final approval, and auto-merge cards go to Discord and the chat Activity feed, not the CTO thread.
...
`usage.est_cost_usd` is a **list-price estimate**. Thinking tokens are billed as output. When Gemini omits `thoughts_token_count` (typical for Pro PR review), billed output is `total_tokens − prompt_tokens`, not visible `candidates_tokens` alone. Each LLM call is logged via `get_llm_client()` as a JSON line with `"event": "llm_usage"` / `"feature": "cto_pr_review"` in Cloud Run logs (no separate review-total line — that would double-count). `fetch_ai_usage` re-estimates from token counts so older log lines that stored a candidates-only cost are corrected. The Discord **done** notification (not the started message) is a single message that starts with `PR:` and `Comment:` so the review body stays tied to the pull request, then appends the cost estimate, e.g. `Estimated LLM cost: ~$0.1800 (gemini-pro-latest, 1 attempt)`.
