...
| `BIGAS_JIRA_AUTOMATION_ALLOWED_PROJECTS` | Default `VFA,WAYW,BIG,REM,GPWW,GPW-PROD,FYDA,MYL,FRI` |
| `BIGAS_JIRA_PROJECT_REPO_MAP` | `VFA:mckort/vcfieldassistant,WAYW:mckort/roadpal,BIG:mckort/bigas,REM:mckort/remotebrief,GPWW:Green-Promo-Wear-Global/greenpromowear-website,GPW-PROD:Green-Promo-Wear-Global/GPW,FYDA:mckort/fulfillyourdreamadventure,MYL:mckort/mylifesdeed,FRI:mckort/frimaninvestments` |
| `BIGAS_PROJECT_LOCAL_PATH_MAP` | Optional `FRI:/path/to/frimaninvestments` for your laptop checkout (GitHub remains the source of truth for cloud agents) |
...
| `GITHUB_TOKEN` | Repo context, Phase 4 PR lookup, and Implement fallback PR create when Cursor `autoCreatePR` fails |
