...
Follow us on X: **[@bigasmyaiteam](https://x.com/bigasmyaiteam)** · Live instance: **[bigas.me](https://bigas.me)**
...
A running instance is at [bigas.me](https://bigas.me). The site is a short overview; this README is the full product and setup guide.
...
| `JIRA_PROJECT_KEY` | Jira project key(s), comma-separated for the whole portfolio (e.g. `VFA,WAYW,BIG,REM,GPWW,GPW-PROD,FYDA,MYL,FRI`). Per-request override via `project_key` / `project_keys`. With `SECRET_MANAGER=true`, update this secret — Cloud Run env is overwritten at startup. GPW-PROD is the store (`store.greenpromowear.com`); GPWW is the marketing site. |
...
3. **`In Progress (AI)`** — Bigas launches a Cursor cloud agent against the repo mapped to this Jira project, waits on that request until the agent finishes (Cloud Run CPU), and opens a pull request. If Cursor cannot auto-create the PR after pushing the branch, Bigas opens it with `GITHUB_TOKEN`. The PR link is commented on the issue; you get pinged in `bigas-cto` and the CTO chat. **Simple tickets can skip steps 1–2:** a title, short brief, and/or screenshot is enough — drag straight from To Do to this column and Cursor implements the smallest matching change.
4. Once the CTO specialist's autofix loop reports the PR is **ready to merge**, Discord pings you. The card stays in **In Progress (AI)** until the PR is **merged** (Bigas auto-merge, GitHub auto-merge after checks, or a human merge). Then Bigas finds the ticket key from the PR title or branch and moves the card to **Final approval (manual)**. How that review/autofix loop works: [from PR to ready to merge](#walkthrough-from-pr-to-ready-to-merge).
...
2. **Clean review** — if Blockers, Important, **and** Minor are empty, Discord says **ready to merge**. Leftover Minor after two nits-only autofix rounds, or at the five-round cap, is also treated as ready. Optional: `BIGAS_CTO_AUTO_MERGE=true` squash-merges from the Actions loop (or enables GitHub auto-merge if checks are still pending). A chat/MCP review does not merge. Squash always uses the **current PR title plus number** (`VFA-59: … (#198)`), so a repo default that prefers the first branch commit cannot drop the ticket key. When the PR is merged (auto-merge or someone else merges), the linked card moves to **Final approval (manual)**. PRs without a key get a board ticket first; that ticket is stamped with the versioned feature branch, the board default, or the lowest unreleased version.
3. **Autofix** — on by default (`BIGAS_AUTO_FIX` defaults to `true`; set `false` to skip). If the review has Blockers/Important, or leftover Minor, `autofix_pr` launches a Cursor cloud agent on the same branch; Actions polls `autofix_followup` until the agent finishes, then Bigas re-reviews (including when the agent finishes without a new commit, so stale findings can be dismissed). Up to five `[bigas-autofix]` rounds; leftover Minor also gets up to two `[bigas-autofix] [nits-only]` rounds, then auto-merge may accept the leftover nits. After five rounds with Blockers/Important still open, Discord/Jira ask you to handle it manually. Autofix commit subjects start with the ticket key (`VFA-53: [bigas-autofix] …`) so prepare-deploy can match git to the board.
...
Fully clean LGTM reviews do not launch autofix. Leftover Minor / nits launch up to two extra autofix rounds; if they remain after those two rounds or the five-round cap, the PR is treated as ready to merge. Post-autofix reviews verify the previous Bigas comment instead of inventing a fresh nit list. Dead/unused code introduced or left unused by the PR is classified as **Important** (so it is fixed before merge), not Minor — but only when the diff itself shows it is unused.
...
**Staging environment.** This is a real stack you build and tear down, not a git branch named `staging-x.y.z`. Type **prepare staging**, **update staging**, **teardown staging**, or **prepare deploy GPW-PROD** (or open **Prepare deploy**, pick the project, and choose the command). With one configured project the project key is optional. With more than one, name it: **prepare staging GPW-PROD**. Prepare staging reviews the candidate branch against production and, when that review is clean, dispatches the project's prepare workflow (for GPW-PROD: build staging from the live production image and a production database copy). Update staging deploys that reviewed SHA, runs migrations, and leaves maintenance on if the post-check fails. Teardown asks, then destroys only that staging stack. Prepare deploy requires the green rehearsal, takes a production backup, asks, then deploys with maintenance on until the post-check is green. A failed update or production deploy starts autofix and does not turn maintenance off.

Projects live in code as `DEFAULT_STAGING_ENVS` (`bigas/resources/devops/gpw_pipeline.py`). `BIGAS_STAGING_ENV_MAP` is a JSON object that adds or replaces entries:

```json
{"DEMO":{"repo":"example/demo","candidate_branch":"develop","production_branch":"main","staging_url":"https://staging.example.com","production_url":"https://example.com","workflows":{"prepare_staging":"prepare-staging.yml","update_staging":"update-staging.yml","teardown":"teardown-staging.yml","backup":"backup-production.yml","deploy_production":"deploy-production.yml"}}}
```

Each product repo owns those workflow files. Bigas only dispatches them. GPW-PROD is the built-in entry (`develop` → staging at `staging.greenpromowear.com`, production branch `production`).

**Versioned prepare-deploy.** Type `prepare deploy VFA 0.1.0` (or use the composer **Prepare deploy** shortcut: project + version). DevOps opens a PR from the feature branch (`staging` for VFA) onto `main` when that branch is ahead (and says so in chat — it never silently skips), puts cut ticket keys on the PR and squash commit, runs CTO review + autofix, and merges when the review is clean. It then posts the tickets on that board release (**Done** and **Final approval** are in the cut — Final approval is tested in prod before Done), compares those keys to commits between prod deploy tags and `main`, shows what `yes` will deploy, and runs a risk check. If the feature branch was not merged because `main` already had it and there is nothing to ship versus prod, it stops instead of asking to deploy empty; when `main` is ahead of prod it still asks, with warnings for cut tickets missing from git. Otherwise it always asks before deploying `main` (even when risk is low and the cut matches git), and calls out leftover open tickets, cut tickets missing from git, or extra commits without a ticket on that version. A **yes** after prepare starts that cut's deploy (leftover social drafts from an earlier version do not steal the confirm). Chat posts Actions run links and per-workflow progress, then post-check. After a green deploy and healthy post-check it closes that board version (GitHub Release `vX.Y.Z`), leaves leftover open cards out of the cut (moves them to the next *existing* unreleased version, or clears the version if none exists), posts customer release notes, and asks whether you want blog/social drafts for that version. **Yes** stores an X Approve/Decline draft (same review page as weekly X posts) and shows other channels as copy only. A plain `deploy VFA` without a version still does not close a release.
...
Bigas includes a **clean, brand-aligned web chat UI** (when the frontend is built). Signed-out visitors see a one-page **marketing landing** at `/` (fork-and-run quickstart, product surfaces, FAQ); sign in at `/login`. After login, `/` is chat. Static SEO assets live in `frontend/public/` (`robots.txt`, `sitemap.xml`) and `frontend/index.html` (Open Graph, Twitter cards, SoftwareApplication JSON-LD). The interface uses the Bigas logo palette (white, `#73cdfb` blue, and black). Chat with your **Chief of Staff** agent, or talk directly to **Marketing**, **Product**, **CTO**, **CFO**, and **DevOps** specialists — each with their own icon. The UI is **mobile-responsive** (phones and small tablets): use the bottom tab bar to switch between Chat, Board, and Objectives. On iOS/Android you can **Add to Home Screen** (PWA) for a standalone app experience.
...
| Endpoint | Description |
|---|---|
| `POST create_ticket` | Create a Task, Feature, Bug, or Improvement on the internal board (or Jira); returns issue key + URL. Shared by every chat agent and MCP client. Use Feature for new user-facing product work; Improvement for polish, UX, copy, and upgrades to existing behavior. Set `marketing=true` for marketing-related tickets (adds label `marketing`). Optional `parent_epic_key` links to a known Epic; omit it for a standalone ticket. Optional `status` sets the column. Alias: `create_jira_issue` |
...
| AI model evaluation (VFA pack) | `0 16 1 * *` (1st of month, Europe/Stockholm) | `.../tasks/eval/vfa-living-analysis` |
...
Bigas periodically benchmarks flagship LLM models (OpenAI, Anthropic, Gemini) against **eval packs** — self-contained YAML files with fixtures, prompts, and optional Bigas-side web research. Each run reads the vendors' model-overview pages ([Claude](https://platform.claude.com/docs/en/models/overview), [OpenAI](https://developers.openai.com/api/docs/models), [Gemini](https://ai.google.dev/gemini-api/docs/models)) and takes **at most two current reasoning models per provider**, then skips ids already eliminated and retests the champion **and the current production baseline**. Packs today: **VC Field Assistant living analysis** (`eval/vfa-living-analysis.pack.yaml`, baseline `gemini:gemini-3.1-pro-preview`, fixtures VC Field Assistant + Stripe) and **Bigas OKR goal loop** (`eval/okr-goal-loop.pack.yaml`, baseline `gemini:gemini-3.8-flash`, frozen Green Promo Wear snapshot). Override the baseline with `EVAL_BASELINE_MODEL`. Bigas runs the models, scores with a **two-judge panel** (Gemini + Claude by default) plus mechanical checks, stores `ranking.json` plus a readable `report.html` / `report.md`, posts a ranking summary to the **Product Manager** chat thread and Discord, and includes a signed clickable report link (`/eval/reports/<use-case>/<run-id>`). The report shows mean score and per-judge scores.

VFA scores the **prompt suite + Bigas web research** (not VFA's citation pipeline). The OKR pack scores the production **tool loop** on a frozen snapshot — not live GA4 or the board.
...
Cloud Scheduler on the **1st of each month at 16:00 Europe/Stockholm** (`0 16 1 * *`). Body `every_n_weeks: 1` runs on every trigger (no in-app biweekly skip). Same auth as evaluate-goals (`X-Bigas-Access-Key` or `CRON_SECRET`):
