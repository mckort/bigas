...
2. **Clean review** — if there are no Blockers/Important, Discord says **ready to merge**. Optional: `BIGAS_CTO_AUTO_MERGE=true` squash-merges (or enables GitHub auto-merge if checks are still pending). Squash always uses the **current PR title plus number** (`VFA-59: … (#198)`), so a repo default that prefers the first branch commit cannot drop the ticket key. When the PR is merged (auto-merge or someone else merges), the linked card moves to **Final approval (manual)**. PRs without a key get a board ticket first; that ticket is stamped with the versioned feature branch, the board default, or the lowest unreleased version.
