...
5. Monday's `weekly_okr_pulse` restates the mechanical scoreboard (including up to **three reasoned next steps per red KR** — gates, open work, stale currents; not “create a task for …” boilerplate), then runs the tool loop on every Objective still in **In Progress (AI)**. It refreshes GA4 currents and may open **To Do** cards for net-new levers. Done work is history, not a stop: a red KR still gets next steps. The loop points at existing `(manual)` gates instead of cloning them, skips near-duplicates and live site/repo evidence, and never auto-starts cards. It never moves an Objective to Final approval or Done until every Key Result has reached its target. You can still edit `current` by hand. The dashboard shows on track / at risk / off track against expected pace. Every Chief, Product, and Marketing chat session is primed with that scoreboard (stale currents, unlinked Done, pending manual gates).
