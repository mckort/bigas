...
LGTM reviews and unstructured nit-only text do not launch autofix. When the structured review has only **### Minor** open (Blockers/Important clear), Bigas runs up to two nits-only autofix rounds (`[bigas-autofix] [nits-only]`). Leftover Minor after those rounds, or at the five-round cap with only Minor open, is treated as **ready to merge** (loop protection still applies when Blockers/Important remain at the cap). Post-autofix reviews verify the previous Bigas comment instead of inventing a fresh nit list. Dead/unused code introduced or left unused by the PR is classified as **Important** (so it is fixed before merge), not Minor — but only when the diff itself shows it is unused.
