...
bigas is a Flask-based MCP server (`app.py`) with three resource blueprints. The server exposes tools in two ways: (1) **HTTP** at `/mcp/tools/*` and `GET /mcp/manifest` for curl and Cloud Scheduler, and (2) **Streamable HTTP MCP** at `POST /mcp` for JSON-RPC (initialize, tools/list, tools/call), used by MCP clients such as Claude, Cursor, and Grok Bot. `GET /mcp` returns 405 (no SSE). New tools registered with `@register_tool` appear in both; no changes to the `/mcp` handler are required when adding providers.
